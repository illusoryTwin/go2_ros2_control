    ...
    def updateStateCommand(self, msg, state, command):
        ...
        if self.use_button:
            if msg.buttons[7]:
                ...
                rclpy.get_logger().info(f"Trot Gait Controller - Use roll/pitch compensation: {self.use_imu}")
                ...
                if not self.autoRest:
                    self.trotNeeded = True
                self.use_button = False
                rclpy.get_logger().info(f"Trot Gait Controller - Use autorest: {self.autoRest}")
